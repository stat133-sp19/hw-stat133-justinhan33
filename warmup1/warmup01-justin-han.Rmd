...
![_Griff Halloran_](https://vignette.wikia.nocookie.net/starwars/images/6/64/SWResistance-GriffHalloran.jpg/revision/latest?cb=20180914204636)
...
***
#Cooking Recipe
_**Baked Buffalo Wings**_

![_Great for all occasion!_](https://smittenkitchendotcom.files.wordpress.com/2019/01/oven-baked-buffalo-wings.jpg?w=1500)
...
$\quad$$\textit{d}$($\textbf{p}$, $\textbf{q}$)= $\textit{d}$($\textbf{q}$, $\textbf{p}$) = $\sqrt{(q_1-p_1)^2+(q_2-p_2)^2+\dots+(q_n-p_n)^2}$
\  

$\hspace{63pt}$ = $\sqrt{\displaystyle\sum_{i=1}^{n}(q_i-p_i)^2}$.
...
$\quad$||**p**|| = $\sqrt{(p_1^2+p_2^2+\dots+p_n^2)}$ = $\sqrt{\textbf{p} \bullet \textbf{q}}$, 
